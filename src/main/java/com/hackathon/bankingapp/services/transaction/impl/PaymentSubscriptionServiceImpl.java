...
public class PaymentSubscriptionServiceImpl implements PaymentSubscriptionService {
    ...
    public PaymentSubscriptionServiceImpl(ThreadPoolTaskScheduler taskScheduler,
    ...
    @Override
    public void scheduleSubscription(PaymentSubscriptionRequest paymentSubscriptionRequest) {
        Long accountId = accountService.getUserAccount().getId();
        Runnable task = () -> this.withDrawMoneyPeriodically(accountId, paymentSubscriptionRequest);

        schedulePayment(accountId, task, paymentSubscriptionRequest.intervalSeconds());
    }
